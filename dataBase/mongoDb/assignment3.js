...
// if you are using nosql booster
// use 'ev'
...
q1 = db.ev_stations_v1.aggregate([
    { $addFields: {
        "LatitudeDouble": { $convert: { input: "$Latitude", to: "double" }},
        "LongitudeDouble": { $convert: { input: "$Longitude", to: "double" }},
    }},
    { $match: { $and: [ 
        { "LatitudeDouble": { $gt: 33.20, $lt: 34.70 }},
        { "LongitudeDouble": { $gt: -118.40, $lt: -117.20 }}
    ]}},
    { $group: {
        "_id": { "ZIP": "$ZIP" },
...
printjson(q1)
// except for the potential typo, answer has been checked with prof's suggested solution
...
// why this one should use ev_stations_v1??? should be electric_vehicle_population?
...
q2 = db.electric_vehicle_population.aggregate([
    { $match: { "Make": "TESLA" }}, 
    // there is no other way to epxress word TESLA
    { $group: {
        "_id": { "State": "$State", "Model": "$Model" },
        "vehicleCount": { $count: { }}
    }},
    { $sort: { "vehicleCount": -1 }}
...
printjson(q2)
// answer has been checked with profs suggested solution
...
q3 = db.electric_vehicle_population.aggregate([
    { $addFields: { "ElectricRangeDouble": { $convert: { input: "$ElectricRange", to: "double" }}}},
    { $group: {
        "_id": { "EVType": "$ElectricVehicleType", "Eligibility": "$CleanAlternativeFuelVehicleEligibility" },
        "avgERange": { $avg: "$ElectricRangeDouble" }
    }},
    { $sort: { "avgERange": -1 }},
    { $project: { "_id": 1, "avgERangeRounded": { $round: [ "$avgERange", 2 ] }}}
])
...
q4 = db.electric_vehicle_population.aggregate([
    { $group: {
        "_id": { "State": "$State", "Make": "$Make", "Model": "$Model" },
        "count_": { $count: { }}
    }},
    // we group the tables first before we join within another grouped table
    { $lookup: {
        from: "co2_emissions_canada",
        let: { make_veh: "$_id.Make", model_veh: "$_id.Model" },
        pipeline: [
            { $addFields: { "CO2Emissions_g_kmDouble": {
                $convert: { input: { $trim: { input: "$CO2Emissions_g_km" }}, to: "double" }
            }}},
            { $group: {
    ...
    { $unwind: {
        path: "$dataFromco2",
        preserveNullAndEmptyArrays: true // default value is false, null value document after unwind will not show up
    }},
    { $sort: { "count_": -1 }}, 
    { $match: { "dataFromco2": { $exists: true }}}
    // { $group: { "_id": null, "count": { $count: { } }}}
])

printjson(q4)
// the count_ is the same as prof's answer after removing the limit: 100
// the actual avg is different as prof only consider the first match pair, while we consider all
...
q5_1 = db.electric_vehicle_population.aggregate([
    ...
    { $lookup: {
        ...
        pipeline: [
            ...
            { $match: { $expr: { $eq: [ "$_id.StateGroup", "$$vp_state" ]}}},
            { $project: { "_id": 0, "EVStationCount": 1 }}
        ],
    ...
    { $project: {
        ...
        "State": "$_id.State",
        "vehicleCount": 1,
        "EVStationCount": "$dataFromEVStations.EVStationCount",
...
// answer has been checked with prof's suggested solution
...
q5_2 = db.electric_vehicle_population.aggregate([
    ...
    { $lookup: {
        ...
        pipeline: [
            ...
            { $match: { $expr: { $eq: [ "$_id.ZipGroup", "$$vp_PostalCode" ]}}},
            { $project: { "_id": 0, "EVStationCount": 1 }}
        ],
        as: "dataFromEVStations"
    }},
...
// answer has been checked with prof's suggested solution
// provided jointed table is not used but rather calculated by ourselves
...
q6 = db.nei_2017_full_data.aggregate([
    { $match: { 
        $and: [   
            ...
            // some description is like "except motor vehicle" they are also removed by our code
        ]
    }},
    { $addFields: { "totalEmissionsDouble": { $convert: { input: "$totalEmissions", to: "double" }
    }}},
    { $group: {
    ...
    { $project: {
        "_id": 0, "naicsDescription": "$_id.naicsDescription", "count_": 1, "sumEmissions": 1, 
        "avgEmissions": 1, "minEmissions": 1, "maxEmissions": 1
    }},
    { $sort: { "sumEmissions": -1 }}
])

printjson(q6)
// besides removing the description like "except motor vehicle", which i believe should not be included
// all the remaining sum result is the same as prof's suggested answer
// max, avg, and min is also provided to facilitate the analysis.
...
q7 = db.nei_2017_full_data.aggregate([
    { $match: { $or: [
        ...
        { "companyName": { $regex: /veatech/, $options: "i" }}
        // if we want, we can also consider the site name...
...
printjson(q7)
// our query is insensitive to case... "companyName":"NUCOR CORPORATION"... will also be included
// prof's query is sensitive to case... which means will only match document that include Nucor exactly
// if we want, we can also consider the site name...

// test different behavior of $regex
...
db.nei_2017_full_data.aggregate([
    { $match: { $or: [
        { "companyName": { $regex: /DANA/}},
        { "companyName": { $regex: /Emerson/}},
        { "companyName": { $regex: /Nucor/}},
    ]}},
    { $group: {
        _id: null,
        count_: {$sum: 1}
