...
from `ev_stations_v1`
where
	(convert(latitude, decimal(10)) > 33.20)
    and
    (convert(latitude, decimal(10)) < 34.70)
    and
    (convert(longitude, decimal(10)) > -118.40)
    and 
    (convert(longitude, decimal(10)) < -117.20)
group by ZIP
...
select state, model, count(*) as countOfVehicle
...
select 
    ...
    avg(convert(ElectricRange, decimal(0))) as avgElectricRange
from electric_vehicle_population
...
from (
	select make, model, 
		avg(convert(CO2Emissions_g_km, decimal(10))) as avgCO2Emissions_g_km,
		max(convert(CO2Emissions_g_km, decimal(10))) as maxCO2Emissions_g_km,
		min(convert(CO2Emissions_g_km, decimal(10))) as minCO2Emissions_g_km
	from co2_emissions_canada
	group by make, model
) as groupCO2
right join (
	select state,make,model, count(*) as countOfVehicle
	from electric_vehicle_population
...
select 
	naicsDescription, 
	count(*) as count_, 
    sum(convert(totalEmissions, decimal(10))) as sumEmissions,
    avg(convert(totalEmissions, decimal(10))) as avgEmissions,
    min(convert(totalEmissions, decimal(10))) as minEmissions,
    max(convert(totalEmissions, decimal(10))) as maxEmissions
...
-- as same mongodb, prof is not removing those with descriptions like "except Automotive"...
-- if removed then same answer
...
from nei_2017_full_data
where
	(companyName like "%dana%") or
    (companyName like "%emerson%") or
    (companyName like "%nucor%") or
    (companyName like "%micron%") or
    (companyName like "%allegheny%") or
    (companyName like "%albemarle%") or
    (companyName like "%schneider%") or
    (companyName like "%veatch%") 
group by state
order by sumEmissions desc;
-- default in sql should be case-insensitives
